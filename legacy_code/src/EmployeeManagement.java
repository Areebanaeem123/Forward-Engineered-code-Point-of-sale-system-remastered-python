...
public class EmployeeManagement{
  ...
  {
	  readFile();
	  int userFound = pos.util.ReturnCode.EMPLOYEE_NOT_FOUND;
	  int index = -1;
	  for(int i=0;i<employees.size();i++){
	        if(username.equals((employees.get(i).getUsername())))
	        {
	          userFound = pos.util.ReturnCode.EMPLOYEE_FOUND;
	          index=i;
	          break;
	        }
	  }
	  
	  if (userFound == pos.util.ReturnCode.EMPLOYEE_FOUND)
	  {
		  if (!(position.equals("Admin")||position.equals("Cashier") || position.equals("")))
			  return pos.util.ReturnCode.INVALID_POSITION;
  ...
  private void readFile(){
    ...
    try {
      ...
      while ((line = textReader.readLine()) != null)
      {
        ...
        String name = lineSort[pos.util.ReturnCode.EMPLOYEE_FIRSTNAME_INDEX] + " " + 
                      lineSort[pos.util.ReturnCode.EMPLOYEE_LASTNAME_INDEX];
        employees.add(new Employee(
            lineSort[pos.util.ReturnCode.EMPLOYEE_USERNAME_INDEX],
            name,
            lineSort[pos.util.ReturnCode.EMPLOYEE_POSITION_INDEX],
            lineSort[pos.util.ReturnCode.EMPLOYEE_PASSWORD_INDEX]));
      }
      textReader.close();
